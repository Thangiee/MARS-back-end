...
class AssistantApiSpec extends ApiSpec {
  ...
  "Instructor user" must {
    "be able to update assistant account info" in {
      instRequest(Post(s"/assistant/${asstBob.netId}", FormData("rate" -> "16.64"))) ~> check {
        instRequest(Get(s"/assistant/${asstBob.netId}")) ~> check {
  ...
  "Assistant user" must {
    ...
    "be able to update their account info" in {
      asstRequest(Post("/assistant", FormData("title" -> "new title"))) ~> check {
        asstRequest(Get("/assistant")) ~> check {
    ...
    "not be able to change their account threshold value" in {
      asstRequest(Post("/assistant", FormData("threshold" -> ".5"))) ~> check(status should equal(Forbidden))
    }
  ...
  "Creating a new assistant account with a username and/or a net id that already exist" must {
    "not delete the existing account with that username" in {
      ...
      def getAsstBob = adminRequest(Get(s"/assistant/${asstBob.netId}"))
